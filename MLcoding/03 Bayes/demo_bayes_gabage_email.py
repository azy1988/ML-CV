...
def spamTest2():
    ...
    for i in np.arange(25):
        print(i)
        temp_pos_string = open('./email/ham/%d.txt' % (i+1)).read()
        temp_neg_string = open('./email/spam/%d.txt' % (i+1)).read()
        temp_pos_string_split = textParse(temp_pos_string)
        temp_neg_string_split = textParse(temp_neg_string)
        docList.append(temp_pos_string_split)
        docList.append(temp_neg_string_split)
        classList.append(1)
        classList.append(0)
        fullText.extend(docList)
    fullVocab = createVocabList(fullText)
    
    
    
    for docIndex in np.arange(18):
        trainMat.append(setOfWords2Vec(fullVocab, docList[docIndex]))
        trainClasses.append(classList[docIndex])
    p0V, p1V, pSpam = trainNB0(np.array(trainMat), np.array(trainClasses))
    
    
    for ii in np.arange(18,25):
        testEntry = docList[ii]
        thisDoc = np.array(setOfWords2Vec(fullVocab, testEntry))
        test_label = classifyNB(thisDoc, p0V, p1V, pSpam)
        if test_label != classList[ii]:
